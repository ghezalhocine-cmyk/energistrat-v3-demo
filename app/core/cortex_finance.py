...
class CortexFinance:
    """
    ENERGISTRAT - CORTEX FINANCE V13.1 TRUE UNICORN
    ...
    # =========================================================
    # 2. MOTEUR D'AUDIT FINOPS & TURPE 7
    # =========================================================
    ...
    def _compute_turpe_7_breakdown(self, inv: Dict[str, Any]) -> Dict[str, Any]:
        ...
        # 2. Heuristique de décomposition
...
# =========================================================
    # 4. MOTEURS DE DÉTECTION (REGEX & HELPERS)
    # =========================================================
    def _empty_invoice_payload(self) -> Dict[str, Any]:
        return {
            "source": "UNKNOWN", "filename": "", "document_hash": "",
            "provider": "INCONNU", "energy_type": "unknown",
            "pdl": None, "pce": None, "invoice_number": None, "invoice_date": None,
            "period_start": None, "period_end": None, "volume_kwh": 0.0, "volume_m3": 0.0,
            "pcs": None, "amount_ht": 0.0, "amount_ttc": 0.0, "taxes_amount": 0.0,
            "penalties": 0.0, "power_subscribed": 0.0, "subscription_amount": 0.0,
            "network_amount": 0.0, "consumption_blocks": {}, "cost_breakdown": {},
            "normalized_output": {}, "raw_preview": ""
        }
    ...
    def _extract_pdl(self, text: str) -> Optional[str]:
        patterns =[r"(?:PDL|POINT DE LIVRAISON|RÉFÉRENCE ACHEMINEMENT|REF ACHEMINEMENT|RÉF EXT|PRM)\s*[:.]?\s*([\d\s]{14,20})"]
    ...
    def _extract_pce(self, text: str) -> Optional[str]:
        patterns =[r"(?:PCE|POINT DE COMPTAGE)\s*[:.]?\s*([\d\s]{14,20})"]
        for pattern in patterns:
            for match in re.finditer(pattern, text, re.IGNORECASE):
                candidate = re.sub(r"\D", "", match.group(1))
                if len(candidate) >= 14:
    ...
    def _build_recommendations(self, inv: Dict[str, Any], anomalies: List[Dict[str, Any]], trust_score: int) -> List[Dict[str, Any]]:
        ...
        if trust_score < 70:
            recommendations.append({"priority": "HIGH", "type": "DATA_QUALITY", "label": "Contrôle manuel recommandé", "next_action": "Relire la facture source et compléter les champs manquants."})
        if any(a.get("label") == "Écart volume SGE" for a in anomalies):
            recommendations.append({"priority": "HIGH", "type": "VOLUME_CHECK", "label": "Comparer facture et données distributeur", "next_action": "Rapprocher le volume facturé avec le relevé distributeur / télérelève."})
        if any(a.get("label") in {"Prix électricité élevé", "Prix gaz élevé"} for a in anomalies):
            recommendations.append({"priority": "HIGH", "type": "PRICING_REVIEW", "label": "Lancer une revue contractuelle", "next_action": "Comparer le prix unitaire à l'offre contractuelle et au benchmark marché."})
        if float(inv.get("penalties") or 0.0) > 0:
            recommendations.append({"priority": "MEDIUM", "type": "PENALTY_REDUCTION", "label": "Réduire les pénalités", "next_action": "Analyser dépassements, énergie réactive ou paramètres de puissance."})

        if not recommendations:
            recommendations.append({"priority": "LOW", "type": "MONITORING", "label": "Facture cohérente", "next_action": "Archiver et poursuivre le monitoring mensuel."})
    ...
    def _to_float(self, value: Any) -> Optional[float]:
        ...
        normalized = str(value).replace("\xa0", " ").replace("€", "").replace("EUR", "").replace(" ", "").strip()
    ...
    # =========================================================
    # 6. SIMULATEURS STRATÉGIQUES (EBITDA & FORECAST)
    # =========================================================
