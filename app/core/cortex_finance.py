...
class CortexFinance:
    VERSION = "V13.1_TRUE_UNICORN"

    def __init__(self) -> None:
        self.logger = logging.getLogger("CortexFinance")
        self.DATA_DIR = os.path.join(os.getcwd(), "data")
        os.makedirs(self.DATA_DIR, exist_ok=True)
        self.known_suppliers =["EDF", "ENGIE", "TOTALENERGIES", "ENI", "VATTENFALL", "GEG", "ALPIQ", "OHM", "DYNEFF", "PRIMAGAZ", "ANTARGAZ", "GAZ DE BORDEAUX", "EKWATEUR", "IBERDROLA", "ELMY", "OCTOPUS", "PLENITUDE", "BP", "SHELL ENERGY"]
        self.tax_keywords =["tva", "cta", "ticfe", "accise", "taxes", "contribution", "cspe", "tcfe", "ticgn", "tvafe", "contribution tarifaire"]
        self.network_keywords =["turpe", "acheminement", "accès réseau", "acces reseau", "distribution", "transport", "atrd", "réseau", "reseau", "part acheminement"]
        self.subscription_keywords =["abonnement", "part fixe", "prime fixe", "terme fixe", "abonnement mensuel", "abonnement annuel", "composante de gestion", "composante de comptage"]
        self.penalty_keywords =["pénalité", "penalite", "dépassement", "depassement", "cos phi", "energie reactive", "énergie réactive", "reactive", "depassement puissance"]
        self.invoice_date_keywords =["date de facture", "date facture", "émise le", "emise le", "invoice date"]
        self._quadrant_aliases = { "HPH":["HPH", "HEURES PLEINES HIVER"], "HCH":["HCH", "HEURES CREUSES HIVER"], "HPE":["HPE", "HEURES PLEINES ETE", "HEURES PLEINES ÉTÉ"], "HCE":["HCE", "HEURES CREUSES ETE", "HEURES CREUSES ÉTÉ"], "HP": ["HP", "HEURES PLEINES"], "HC":["HC", "HEURES CREUSES"], "BASE": ["BASE"], "POINTE": ["POINTE"] }
    ...
    def audit_invoice(self, invoice_wrapper: Dict[str, Any], site_data: Dict[str, Any]) -> Dict[str, Any]:
        ...
        if amount_ttc > 0 and amount_ht > amount_ttc: anomalies.append(self._anomaly("HIGH", "Montants incohérents", "Le montant HT est supérieur au TTC."))
        ...
        if not invoice_date: anomalies.append(self._anomaly("LOW", "Date de facture absente", "Date d'émission non détectée."))
        ...
        if volume_kwh > 0 and sge_real_kwh > 0:
            ...
            if abs(delta_pct) > 5: anomalies.append(self._anomaly("HIGH", "Écart volume SGE", f"Facturé: {round(volume_kwh)} kWh vs Référence: {round(sge_real_kwh)} kWh.", {"delta_pct": round(delta_pct, 2)}))
        ...
        if penalties > 0: anomalies.append(self._anomaly("MEDIUM", "Pénalités", f"{round(penalties, 2)} € de pénalités/dépassements détectés."))
    ...
    def _parse_pdf_native(self, content: bytes, data: Dict[str, Any]) -> Dict[str, Any]:
        try:
            ...
            if data["amount_ht"] <= 0 and data["amount_ttc"] > 0: data["amount_ht"] = round(data["amount_ttc"] / 1.2, 2)
            ...
            if data["volume_kwh"] <= 0 and data["volume_m3"] > 0 and data["pcs"]: data["volume_kwh"] = round(data["volume_m3"] * data["pcs"], 2)
        ...
        except Exception as exc: return {"status": "ERROR", "message": f"Erreur PDF : {str(exc)}"}
    ...
    def _extract_pdl(self, text: str) -> Optional[str]:
        for pattern in[r"(?:PDL|POINT DE LIVRAISON|RÉFÉRENCE ACHEMINEMENT|PRM)\s*[:.]?\s*([\d\s]{14,20})"]:
            for match in re.finditer(pattern, text, re.IGNORECASE):
                ...
                if len(candidate) >= 14: return candidate[:14]
        return None
    ...
    def _extract_invoice_number(self, text: str) -> Optional[str]:
        for pattern in[r"(?:N°\s*DE\s*FACTURE|NUM[ÉE]RO\s*DE\s*FACTURE|FACTURE|INVOICE)\s*[:#]?\s*([A-Z0-9\-_/]{4,})", r"(?:RÉFÉRENCE\s*FACTURE|REFERENCE\s*FACTURE)\s*[:#]?\s*([A-Z0-9\-_/]{4,})"]:
    ...
    def _extract_dates_range(self, text: str) -> Tuple[Optional[str], Optional[str]]:
        ...
        patterns =[r"du\s+(\d{2}[/.-]\d{2}[/.-]\d{2,4})\s+au\s+(\d{2}[/.-]\d{2}[/.-]\d{2,4})", r"p[ée]riode\s*[:.]?\s*(\d{2}[/.-]\d{2}[/.-]\d{2,4})\s*(?:au|-|à)\s*(\d{2}[/.-]\d{2}[/.-]\d{2,4})", r"consommation\s+du\s+(\d{2}[/.-]\d{2}[/.-]\d{2,4})\s+au\s+(\d{2}[/.-]\d{2}[/.-]\d{2,4})"]
        for pattern in patterns:
    ...
    def _extract_amount_by_keywords(self, text: str, keywords: List[str]) -> Optional[float]:
        ...
        for keyword in keywords:
            match = re.search(rf"{re.escape(keyword)}[^\n\r]{{0,100}}?(-?\d[\d\s.,]+)\s*€", normalized, re.IGNORECASE)
    ...
    def _extract_power(self, text: str) -> float:
        patterns = [r"(\d[\d.,]+)\s*kva", r"puissance\s+souscrite\s*[:.]?\s*(\d[\d.,]+)"]
        for pattern in patterns:
            match = re.search(pattern, text, re.IGNORECASE)
    ...
    def _extract_taxes(self, text: str, amount_ht: float, amount_ttc: float) -> float:
        value = self._extract_amount_by_keywords(text,["total taxes", "taxes", "tva", "cspe", "ticfe", "ticgn", "cta"])
    ...
    def _extract_consumption_blocks(self, text: str, energy_type: str) -> Dict[str, Any]:
        ...
        blocks: Dict[str, Any] = {}
        generic_patterns =[r"(HPH|HCH|HPE|HCE|HP|HC|POINTE|BASE)[^\n]*?([\d\s.,]+)\s*kWh[^\n]*?([\d\s.,]+)\s*€"]
        for pattern in generic_patterns:
            for raw_label, raw_volume, raw_cost in re.findall(pattern, text, re.IGNORECASE):
                ...
                if volume > 0: blocks[label] = { "volume_kwh": round(volume, 2), "cost_ht": round(cost, 2), "pmp_eur_kwh": round(cost / volume, 6) if volume > 0 else 0.0 }
        if blocks: return blocks
        for canonical, aliases in self._quadrant_aliases.items():
            for alias in aliases:
                match = re.search(rf"{re.escape(alias)}[^\n]{{0,80}}?(\d[\d\s.,]+)\s*kWh", text, re.IGNORECASE)
                if match:
                    volume = self._to_float(match.group(1)) or 0.0
                    if volume > 0: blocks[canonical] = { "volume_kwh": round(volume, 2), "cost_ht": 0.0, "pmp_eur_kwh": 0.0 }
        return blocks
    ...
    def _build_recommendations(self, inv: Dict[str, Any], anomalies: List[Dict[str, Any]], trust_score: int) -> List[Dict[str, Any]]:
        ...
        if trust_score < 70: recommendations.append({ "priority": "HIGH", "type": "DATA_QUALITY", "label": "Contrôle manuel recommandé", "next_action": "Relire la facture source et compléter les champs manquants." })
        if any(a.get("label") == "Écart volume SGE" for a in anomalies): recommendations.append({ "priority": "HIGH", "type": "VOLUME_CHECK", "label": "Comparer facture et données distributeur", "next_action": "Rapprocher le volume facturé avec le relevé distributeur / télérelève." })
        if any(a.get("label") in {"Prix électricité élevé", "Prix gaz élevé"} for a in anomalies): recommendations.append({ "priority": "HIGH", "type": "PRICING_REVIEW", "label": "Lancer une revue contractuelle", "next_action": "Comparer le prix unitaire à l'offre contractuelle et au benchmark marché." })
        if float(inv.get("penalties") or 0.0) > 0: recommendations.append({ "priority": "MEDIUM", "type": "PENALTY_REDUCTION", "label": "Réduire les pénalités", "next_action": "Analyser dépassements, énergie réactive ou paramètres de puissance." })
        if not recommendations: recommendations.append({ "priority": "LOW", "type": "MONITORING", "label": "Facture cohérente", "next_action": "Archiver et poursuivre le monitoring mensuel." })
    ...
    def _build_normalized_output(self, data: Dict[str, Any]) -> Dict[str, Any]:
        ...
        return {
            ...
            "pricing": {
                "unit_price_eur_kwh": round((float(data.get("amount_ht") or 0.0) / float(data.get("volume_kwh") or 1.0)), 6) if float(data.get("volume_kwh") or 0.0) > 0 else 0.0,
                "unit_price_eur_mwh": round((float(data.get("amount_ht") or 0.0) / float(data.get("volume_kwh") or 1.0)) * 1000, 2) if float(data.get("volume_kwh") or 0.0) > 0 else 0.0,
                "consumption_blocks": data.get("consumption_blocks", {}), "cost_breakdown": data.get("cost_breakdown", {})
            }
        }
    ...
    # =========================================================
    # 5. SIMULATEURS HISTORIQUES (POUR RETRO-COMPATIBILITÉ)
    # =========================================================
    ...
    def simulate_landing(self, site_data: Dict[str, Any]) -> Dict[str, Any]:
        try:
            ...
            if annual_mwh <= 0 and annual_kwh > 0: annual_mwh = annual_kwh / 1000.0

            current_price_eur_mwh = float(contract.get("price_eur_mwh") or contract.get("px_hedged") or contract.get("unit_price_eur_mwh") or 0.0)
            market_price_eur_mwh = float(settings.get("market_price_eur_mwh") or kpis.get("market_price_eur_mwh") or current_price_eur_mwh or 0.0)
            ...
            trajectory =[{"month": month, "baseline_eur": round(monthly_run_rate_eur, 2), "optimized_eur": round(optimized_monthly, 2), "delta_eur": round(monthly_run_rate_eur - optimized_monthly, 2)} for month in range(1, 13)]

            return { "status": "SUCCESS", "year": year, "landing_euro": round(baseline_total_eur, 2), "optimized_landing_euro": round(optimized_landing_eur, 2), "market_reference_euro": round(market_reference_eur, 2), "potential_savings_euro": round(potential_savings_eur, 2), "volume_mwh": round(annual_mwh, 3), "unit_price_eur_mwh": round(current_price_eur_mwh, 2), "trajectory": trajectory }
        except Exception as exc:
            self.logger.error("Erreur simulate_landing : %s", exc)
            return { "status": "ERROR", "message": str(exc), "year": datetime.now().year, "landing_euro": 0.0, "trajectory": [] }

    def healthcheck(self) -> Dict[str, Any]:
        return { "status": "OK", "engine": "CortexFinance", "pdf_ready": PDF_READY, "known_suppliers": len(self.known_suppliers), "version": self.VERSION }
