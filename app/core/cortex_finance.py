...
class CortexFinance:
    """
    ENERGISTRAT - CORTEX FINANCE V13.0 TRUE UNICORN
    - Extraction Heuristique PDF / XML (Factur-X)
    - Décomposition analytique TURPE 7 (CG, CC, CS, CMDPS)
    - Fiscalité légale (CTA, Accise volumétrique)
    - Recommandations opérationnelles FinOps
    """

    VERSION = "V13.0_TRUE_UNICORN"
    ...
    # =========================================================
    # 2. MOTEUR D'AUDIT FINOPS (BAP, SCORE & TURPE 7)
    # =========================================================
    def audit_invoice(self, invoice_wrapper: Dict[str, Any], site_data: Dict[str, Any]) -> Dict[str, Any]:
        ...
        if not invoice_date:
            anomalies.append(self._anomaly("LOW", "Date de facture absente", "Date d'émission non détectée."))
        ...
        if volume_kwh > 0 and sge_real_kwh > 0:
            ...
            if abs(delta_pct) > 5:
                anomalies.append(self._anomaly("HIGH", "Écart volume SGE", f"Facturé: {round(volume_kwh)} kWh vs Référence: {round(sge_real_kwh)} kWh.", {"delta_pct": round(delta_pct, 2)}))
        ...
        if penalties > 0:
            anomalies.append(self._anomaly("MEDIUM", "Pénalités", f"{round(penalties, 2)} € de pénalités/dépassements détectés."))
        ...
        # --- MOTEUR TURPE 7 (LEGAL DECOMPOSITION) ---
    ...
    def _compute_turpe_7_breakdown(self, inv: Dict[str, Any]) -> Dict[str, Any]:
        """Décomposition analytique d'après les règles CRE TURPE 7 HTA/BT"""
        ...
        # 2. Heuristique de décomposition des montants facturés
        cg = subscription * 0.20 # Composante de Gestion
        cc = subscription * 0.15 # Composante de Comptage
        cs_fixed = subscription * 0.65 # Composante Soutirage (Partie Puissance b_i)

        cs_variable = network * 0.95 # Composante Soutirage (Partie Énergie c_i)
        cmdps = float(inv.get("penalties") or 0.0) # Dépassements
        
        # 3. Fiscalité légale
        # CTA (21.93% assis sur la part fixe)
        cta_estimated = (cg + cc + cs_fixed) * 0.2193
        # Accise volumétrique (21€/MWh pour 2025/2026)
        accise_estimated = (vol_kwh / 1000.0) * 21.0

        return {
            "domain": domain,
            "components": {
                "CG_gestion": round(cg, 2),
                "CC_comptage": round(cc, 2),
                "CS_fixe": round(cs_fixed, 2),
                "CS_variable": round(cs_variable, 2),
                "CMDPS_depassement": round(cmdps, 2)
            },
            "taxes_legales": {
                "CTA_estimee": round(cta_estimated, 2),
                "Accise_estimee": round(accise_estimated, 2)
            },
            "turpe_total": round(subscription + network + cmdps, 2)
        }
        
        # =========================================================
    # 3. EXTRACTION PDF / XML (AVEC SÉCURITÉ CLOUD RUN)
    # =========================================================
        ...
        try:
            ...
            with pdfplumber.open(io.BytesIO(content)) as pdf:
                for i, page in enumerate(pdf.pages):
                    if i >= 3: # 🛡️ Protection OOM Kill Cloud Run
            ...
            data["raw_preview"] = text[:1000] # Limité à 1000 carac pour soulager Firestore
            ...
            data["amount_ttc"] = self._extract_amount_by_keywords(text, ["total ttc", "montant ttc", "net à payer ttc"]) or 0.0
    ...
    # =========================================================
    # 4. MOTEURS DE DETECTION / SCORING
    # =========================================================
    ...
    def _extract_invoice_number(self, text: str) -> Optional[str]:
        for pattern in[r"(?:N°\s*DE\s*FACTURE|FACTURE|INVOICE)\s*[:#]?\s*([A-Z0-9\-_/]{4,})", r"(?:RÉFÉRENCE\s*FACTURE)\s*[:#]?\s*([A-Z0-9\-_/]{4,})"]:
    ...
    def _extract_invoice_date(self, text: str) -> Optional[str]:
        patterns = [rf"{re.escape(k)}\s*[:.]?\s*(\d{{2}}[/.-]\d{{2}}[/.-]\d{{2,4}})" for k in self.invoice_date_keywords]
        for pattern in patterns:
            match = re.search(pattern, text, re.IGNORECASE)
            if match:
                parsed = self._parse_fr_date(match.group(1))
    ...
    def _extract_dates_range(self, text: str) -> Tuple[Optional[str], Optional[str]]:
        ...
        return min(p[0] for p in dates).strftime("%Y-%m-%d"), max(p[1] for p in dates).strftime("%Y-%m-%d")
    ...
    def _extract_penalties(self, text: str) -> float:
        return round(sum(self._extract_amount_by_keywords(text, [k]) or 0.0 for k in self.penalty_keywords), 2)

    def _extract_consumption_blocks(self, text: str, energy_type: str) -> Dict[str, Any]:
        if energy_type == "gas": return {}
        blocks: Dict[str, Any] = {}
        for raw_label, raw_volume, raw_cost in re.findall(r"(HPH|HCH|HPE|HCE|HP|HC|POINTE|BASE)[^\n]*?([\d\s.,]+)\s*kWh[^\n]*?([\d\s.,]+)\s*€", text, re.IGNORECASE):
            vol = self._to_float(raw_volume) or 0.0
            cost = self._to_float(raw_cost) or 0.0
            if vol > 0: blocks[raw_label.upper()] = { "volume_kwh": round(vol, 2), "cost_ht": round(cost, 2), "pmp_eur_kwh": round(cost / vol, 6) }
        return blocks
    ...
    def _build_cost_breakdown(self, data: Dict[str, Any]) -> Dict[str, Any]:
        ...
        return {
            ...
            "network": round(net, 2),
            "energy_variable": round(max(ht - sub - net - pen, 0.0), 2),
            "taxes": round(tax, 2),
    ...
    def _compute_trust_score(self, inv: Dict[str, Any], anomalies: List[Dict[str, Any]]) -> Tuple[int, Dict[str, int]]:
        ...
        # 1. Base Score = moyenne des critères de présence (La correction du Bug)
        base_score = int(sum(field_scores.values()) / len(field_scores)) if field_scores else 100
        
        # 2. Soustraction des pénalités d'anomalies
    ...
    def _build_recommendations(self, inv: Dict[str, Any], anomalies: List[Dict[str, Any]], trust_score: int) -> List[Dict[str, Any]]:
        ...
        if trust_score < 70:
            recommendations.append({"priority": "HIGH", "type": "DATA_QUALITY", "label": "Contrôle manuel recommandé", "next_action": "Relire la facture source."})
        if any(a.get("label") == "Écart volume SGE" for a in anomalies):
            recommendations.append({"priority": "HIGH", "type": "VOLUME_CHECK", "label": "Comparer facture et données distributeur", "next_action": "Rapprocher le volume facturé."})
        if any(a.get("label") in {"Prix électricité élevé", "Prix gaz élevé"} for a in anomalies):
            recommendations.append({"priority": "HIGH", "type": "PRICING_REVIEW", "label": "Lancer une revue contractuelle", "next_action": "Comparer le prix unitaire."})
        if float(inv.get("penalties") or 0.0) > 0:
            recommendations.append({"priority": "MEDIUM", "type": "PENALTY_REDUCTION", "label": "Réduire les pénalités", "next_action": "Analyser dépassements ou énergie réactive."})
        if not recommendations:
            recommendations.append({"priority": "LOW", "type": "MONITORING", "label": "Facture cohérente", "next_action": "Archiver."})
    ...
    def _build_normalized_output(self, data: Dict[str, Any]) -> Dict[str, Any]:
        return {
            ...
            "metering": { "identifier": data.get("pdl") or data.get("pce"), "power_subscribed_kva": data.get("power_subscribed") },
            "period": { "start": data.get("period_start"), "end": data.get("period_end") },
            "quantities": { "volume_kwh": round(float(data.get("volume_kwh") or 0.0), 2) },
    ...
    def simulate_ebitda(self, ca_k_eur: float, marge_pct: float, gains_eur: float, multiple: float) -> Dict[str, Any]:
        try:
            ...
            return { "status": "SUCCESS", "val_creation_eur": round(val_creation, 2), "nouvelle_marge_pct": round(nouvelle_marge, 2), "equivalent_ca_eur": round((float(gains_eur) / marge_decimal) if marge_decimal > 0 else 0.0, 2) }
        except Exception as exc:
