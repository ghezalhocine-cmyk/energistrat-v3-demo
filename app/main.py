...
app = FastAPI(title="ENERGISTRAT V3", version="EMPIRE-V12.3-ACADEMY")
...
async def api_save_academy_progress(payload: AcademyProgressModel, user = Depends(get_current_user)):
    ...
    if not user: return JSONResponse({"error": "Non autorisé"}, 401)
    profile = db.get_user_profile(user.get("uid"))
    profile["academy"] = {"xp": payload.xp, "badges": payload.badges}
...
async def get_fleet_data(response: Response, user = Depends(get_current_user)):
    ...
    for s in filtered_sites:
        ...
        fleet_list.append({
            "id": get_safe_id(s.get('identity', {}).get('id')), "name": fin.get('meta', {}).get('site_label', 'Inconnu'), 
            "city": city, "zip": s.get('location', {}).get('zip_code', ''), "volume": final_vol, 
            "energy": "gaz" if fin.get('meta', {}).get('is_gas') else "elec", "segment": contract.get('segment', '-'),
            "provider": prov, "budget": final_budget, "landing": fin.get('landing_forecast', 0), 
            "alert": fin.get('kpis', {}).get('pmc_eur_mwh', 0) > 300, "ghost_savings": fin.get('kpis', {}).get('ghost_savings', 0), 
            "power": contract.get('power', 0), "pdl": contract.get('pdl') or contract.get('pce', '-'), 
            "surface": s.get('location', {}).get('surface', 0), "tenant_id": s.get('identity', {}).get('tenant_id', 'Orphelin'),
...
async def api_finance_upload(file: UploadFile = File(...), site_id: str = Form(...), user = Depends(get_current_user)):
    try:
        ...
        if user.get("role") != "ADMIN" and site_data.get("identity", {}).get("tenant_id") != db.get_user_profile(user.get("uid")).get("tenant_id"): return JSONResponse({"error": "Accès refusé"}, 403)
